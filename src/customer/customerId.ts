 ...
 * @param customerId - The ID value provided by the platform
 * @param emailAddress - an email address, if any is available
 * @returns either (a) the original customer Id, if set or (b) a unique UUID based on the email address or (c) a random, unique UUID
 */
export const getUniqueCustomerId = (customerId = '', emailAddress = ''): string => {
  let id = customerId;
